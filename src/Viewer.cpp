...
namespace DDG
{
   ...
   bool Viewer::renderWireframe = false;
    bool Viewer::showVectorField = false;
   ...
   {
       if( glutGetModifiers() && (GLUT_ACTIVE_SHIFT || GLUT_ACTIVE_ALT || GLUT_ACTIVE_CTRL)) {
           if (glutGetModifiers() == GLUT_ACTIVE_SHIFT && state == GLUT_UP)
               pickVertex(x, y, true);
           else if (glutGetModifiers() == GLUT_ACTIVE_ALT && state == GLUT_UP)
               pickVertex(x, y, false);
           else if (glutGetModifiers() == GLUT_ACTIVE_CTRL && state == GLUT_UP)
               pickFace(x, y);
       } else {
   ...
   void Viewer :: drawWireframe( void )
   {
    ...
    {
        std::cout << "drawing vector field" << std::endl;
        shader.disable();
        glPushAttrib( GL_ALL_ATTRIB_BITS );
        ...
        for( EdgeCIter e  = mesh.edges.begin();
            ...
            e ++ )
        {
            Vector midpoint = (e->he->vertex->position);// + e->he->flip->vertex->position) / 2;
            Vector endpoint = midpoint + e->vec;
            glVertex3dv( &midpoint[0] );
   ...
   void Viewer :: drawSelectedVertices( void )
   {
    ...
    void Viewer :: drawSelectedFaces( void )
    {
        glPushAttrib( GL_ALL_ATTRIB_BITS );
            ...
            f ++ )
        {
            if( f->vortexTag )
            {
                ...
                glPushMatrix();
                glTranslated(f->barycenter().x, f->barycenter().y, f->barycenter().z);
                glutSolidSphere(h, 10, 10);
                glPopMatrix();
            }
        }
   ...
   void Viewer :: pickVertex(int x, int y, bool source)
   {
      ...
      {
          if (source) {
              if (mesh.vertices[index].sinkTag)
                  mesh.vertices[index].toggleSinkTag();
              mesh.vertices[index].toggleSourceTag();
          } else {
              if (mesh.vertices[index].sourceTag)
                  mesh.vertices[index].toggleSourceTag();
              mesh.vertices[index].toggleSinkTag();
          }
         updateDisplayList();
    ...
    void Viewer :: pickFace(int x, int y)
    {
        int width  = glutGet(GLUT_WINDOW_WIDTH );
        int height = glutGet(GLUT_WINDOW_HEIGHT);
        if( x < 0 || x >= width || y < 0 || y >= height ) return;
        
        int bufSize = mesh.faces.size();
        GLuint* buf = new GLuint[bufSize];
        glSelectBuffer(bufSize, buf);
        ...
        if (index >= 0)
        {
            mesh.faces[index].toggleVortexTag();
            updateDisplayList();
        }
